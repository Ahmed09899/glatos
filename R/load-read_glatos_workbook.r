...
read_glatos_workbook <- function(
  ...
  if (wb_version %in% c("1.3", "1.4")) {
    ...
    wb <- setNames(vector("list", length(sheets_to_read)), 
    ...
    for(i in 1:length(sheets_to_read)) {
      ...
      sheet_i <- 
        tryCatch(
          readxl::read_excel(
            ...
            range = cellranger::cell_limits(ul = c(xl_first_row, 1),
                                            lr = c(NA, xl_last_col)),
            col_types = "list",
            na = c("", "NA"),
            n_max = 0,
      ...
      if(inherits(sheet_i, "error")) stop("In workbook '", basename(wb_file), 
                                        "' sheet '", sheets_to_read[i], "':",
                                        "\n ", sheet_i$message,
                                        call. = FALSE)
      ...
      if(length(dupl_cols) > 0) 
        err_i <- 
          c(err_i,
            paste0("In sheet '",  sheets_to_read[i], "':",
      ...
      if(nrow(sheet_i) > 0){
        ...
        for(j in char_cols) sheet_i2[[j]] <- cast(sheet_i[[j]], 
                                                  new_class = "character")
        ...
        for(j in num_cols) sheet_i2[[j]] <- cast(sheet_i[[j]], 
                                                 new_class = "numeric")
...
cast <- function(x, 
  ...
  invalid_new_class <- setdiff(new_class,
                               eval(formals(cast)$old_class))
  
  if(length(invalid_new_class) > 0) stop("Invalid 'new_class': \"",
                                         paste0(invalid_new_class,
                                                collapse = ", "), "\"")
  
  # Check if old_class is valid 
  # (only default values of old_class are supported)
  invalid_old_class <- setdiff(old_class,
                                   eval(formals(cast)$old_class))
  
  if(length(invalid_old_class) > 0) stop("Invalid 'old_class': \"",
  ...
  x2 <- suppressWarnings(
    lapply(seq_along(x), function(k){
      if(inherits(x[[k]], old_class)){
        ...
        tryCatch(do.call(cast_with, c(list(x = x[[k]]), 
                                      args_in_k)),
  ...
  print_first_n <- function(x, n = 3){
    if(length(x) <= n) { 
      ...
      paste0(paste(x[1]:min(length(x), n), collapse = ", "), 
             ", ...",
             tail(x, 1),
             " (+ ", length(x) - n - 1, " others)")
    }
  ...
  if(any(cast_status == 3)){
    ...
    error_input_class_skipped <- c(error_input_class_skipped, 
      paste0("row", plural, " ",
  ...
  if(any(cast_status == 4)){
    ...
    error_cast_failed <- c(error_cast_failed, 
      paste0("row", plural, " ", 
  ...
  if(any(cast_status == 2)){
    ...
    warning_cast_to_check <- c(warning_cast_to_check, 
      paste0("row", plural, " ", 
...
identify_workbook_version <- function(wb_file, 
  ...
  if (all(names(glatos_workbook_schema$v1.3) %in% sheets)) {
    ...
    if(all(prj[[1]][1:3] == c("GLATOS Project Code:", 
  ...
  if(is.null(wb_version) == FALSE){
    ...
    if(wb_version != wbv) stop("Workbook version looks like ",
